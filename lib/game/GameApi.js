...
export const usePowerUp = (gameId, playerId, powerUp) => {
    ...
    return fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/game/${gameId}/use/${powerUp}`,
        requestOptions
    ).then(async (res) => {
        if (!res.ok) {
            return Promise.reject(
                new Error(`error on requesting /game/${gameId}/use/${powerUp}`)
            );
        }
...
export const tiltGame = (direction, playerId, gameId) => {
    ...
    return fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/game/${gameId}/tilt`,
        requestOptions
    ).then(async (res) => {
        if (!res.ok) {
            return Promise.reject(
...
export const replacePuck = (gameId) => {
    ...
    return fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/game/${gameId}/replace`,
        requestOptions
    ).then(async (res) => {
        if (!res.ok) {
            return Promise.reject(
...
export const pendingGames = () => {
    return fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/game/pending`).then(
        async (res) => {
...
export const joinGame = (playerName, gameId) => {
    ...
    return fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/game/${gameId}/join`,
        requestOptions
    ).then(async (res) => {
        if (!res.ok) {
...
export const getGame = (gameId) => {
    return fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/game/${gameId}`).then(
        async (res) => {
            const data = await res.json();
            if (!res.ok) {
                const error = (data && data.message) || res.status;
